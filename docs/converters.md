...
All public methods return `mediavocab` objects directly. No explicit conversion step is needed.
...
| `release.image` | `str` | `artwork_url` or uploader `avatar_url`; `""` when absent |
| `release.stream_mode` | `StreamMode` | Always `StreamMode.ON_DEMAND` |
| `release.codec` | `str` | Best transcoding `mime_type` (e.g. `"audio/mpeg"`) |
| `release.bitrate` | `str` | `"256"` (hq) or `"128"` (sq) from transcoding quality |
| `release.audio_channels` | `str` | `"stereo"` when codec known, else `""` |
| `release.license` | `str` | SPDX id (see table below) or raw SC value |
| `release.release_date` | `str \| None` | ISO date from `display_date` or `created_at` |
| `release.external_ids["soundcloud_track_id"]` | `str` | SC track numeric id |
| `release.external_ids["soundcloud_user_id"]` | `str` | SC uploader numeric id |
...
| `release.work.credits[0].relation_role` | `RelationRole` | `PERFORMER` (tracks) / `CREATOR` (sets) |
...
`release.external_ids["soundcloud_playlist_id"]` is also populated.

Fields marked `SoundCloudAPI only` are empty (`""`, `None`, `[]`) when the HTML
backend is used, because SoundCloud's search and track HTML does not expose them.
...
| `entity.external_ids["soundcloud_user_id"]` | `str` | SC user numeric id |
...
Combines SC `genre` field and `tag_list` (space-separated, quoted multi-word).
Each token is looked up against `mediavocab.taxonomy.genre.GENRE_*` constants
(trying the token, its `_`-normalised form, and `&` → `and` variant).
Unrecognised tokens are kept verbatim. Duplicates are discarded.
